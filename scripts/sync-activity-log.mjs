...
function deriveState(events) {
  ...
  // Orphan filter (2026-08-15). For ~25 minutes the vault-side hook stamped a
  // recurring ~31s-per-in-flight-subagent tick (a SubagentStop-shaped event with
  // no agent_type and no transcript) as "Agent: unknown-agent" before it learned
  // to reject them. 41 such invocations are permanently in events.jsonl: the log
  // is append-only by contract AND git-tracked with `merge=union`, so truncating
  // it can be silently undone by a merge from any branch still holding the lines.
  // Skipping on read is the safe fix. Collect ids from the agent_status rows so
  // the paired terminal_log rows drop too, rather than string-matching two
  // different fields. Exact-match on name, never a substring — a future
  // genuinely-unnamed agent should surface, not be swallowed by this.
  const ORPHAN_NAME = 'Agent: unknown-agent'
  const orphanIds = new Set(
    events
      .filter((e) => e.type === 'agent_status' && e.name === ORPHAN_NAME)
      .map((e) => e.agentId),
  )

  for (const ev of events) {
    if (orphanIds.has(ev.agentId)) continue
    switch (ev.type) {
      case 'agent_status':
